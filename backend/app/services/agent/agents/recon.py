...
2. **至少调用三个工具** - 优先使用 rag_query 语义搜索关键入口；若 RAG 返回 401/Unauthorized，必须改用 search_code + read_file
...
```
若 RAG 认证失败（401）时，必须立即降级：
```
Thought: RAG 不可用，改用精确代码搜索定位关键函数
Action: search_code
Action Input: {"keyword": "router.", "file_pattern": "*.js", "max_results": 30}
```
**或者**仅查看根目录结构：
```
...
class ReconAgent(BaseAgent):
    ...
    def _is_rag_auth_failure(tool_name: str, observation: str) -> bool:
        ...
        return any(marker in obs for marker in markers)

    @staticmethod
    def _build_search_code_fallback_hint() -> str:
        """RAG 401 时的降级指引"""
        return (
            "⚠️ 系统提示: RAG 工具认证失败（401/Unauthorized）。\n"
            "请立即改用 search_code 精确定位关键函数，并用 read_file 校验上下文：\n"
            "1. JavaScript/TypeScript: keyword 可用 router., app.get(, express.Router, jwt.verify\n"
            "2. Solidity: keyword 可用 function , delegatecall, tx.origin\n"
            "3. 必须配合 file_pattern 缩小范围，例如 *.js / *.ts / *.sol\n"
            "4. 命中后马上 read_file 读取周边行，提取真实入口点与高风险区域"
        )
...
Final Answer: [JSON格式的结果]"""
                ...
                if step.action:
                    ...
                    if rag_auth_failed:
                        self._conversation_history.append({
                            "role": "user",
                            "content": "请立即改用 search_code（带 file_pattern）精确定位关键函数，再用 read_file 获取上下文。",
                        })
                else:
                    # LLM 没有选择工具，提示它继续
